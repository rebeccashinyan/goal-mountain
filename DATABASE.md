...
| `pending_revision` | `{ schedule, focus_area, priority_recommendation, note, diff, created_at }` — a proposed plan change awaiting the user's decision. Written by `/api/plan/steer` (whole open-days rewrite) or `/api/plan/update` (structured task-level edit, when it's too large/ambiguous to apply directly) on **drafts and active weeks alike**, and cleared by `POST /api/plan/revision`. While present, the live `schedule` is authoritative and untouched |
...
| `setup` | `{ available_time?, user_constraints? }` — what the user told the Planning Agent when this week was generated, saved by `POST /api/plan` so **Change plan setup** can pre-fill the form with their real answers instead of an empty one. Absent when they generated without filling anything in |
...
- **Discarding a draft (`DELETE /api/plan?plan_id=…`) deletes every draft row for that mountain+week**, not just the newest — otherwise a superseded draft underneath would simply resurface and the week wouldn't look discarded. It refuses to touch an active plan (started weeks are real history), skips the query entirely when the id list is empty, and is scoped by `mountain_id` + `week_start` *and* explicit ids so it cannot reach past the one week it's discarding.
...
- `POST /api/plan/steer` (quick actions + guide requests), `POST /api/plan/revision` (apply/discard a proposed change), `POST /api/plan/update` (the Guide Agent's structured add/remove/move/update/replace task edits — validated deterministically in `applyPlanOperations()`/`lib/plans.ts`, then either written straight to `schedule` for a small explicit change or into `pending_revision` for anything larger/ambiguous), `POST /api/plan/replace-task` (the two-step AI-assisted single-task Replace flow — directions, then a concrete task), and `POST /api/plan/rebase` (what "Start this week" actually calls — rescues any already-passed tasks before activating) are companions to `POST /api/plan` — see AGENTS.md → Planning + Strategy Agent. None of them insert rows: `steer`, `revision`, `update`, and `rebase` update the existing plan in place, and `replace-task` is stateless (it reads `weekly_plans` by `plan_id` for schedule context, but the client applies the confirmed replacement via `PATCH /api/plan`).
...
| DELETE | `/api/plan` | weekly_plans (deletes every DRAFT row for one mountain+week) | weekly_plans |
...
| POST | `/api/plan/update` | weekly_plans (applies directly, or as a pending revision) | weekly_plans |
