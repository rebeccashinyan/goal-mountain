...
Weekly plan display for the Overview page. Reads from `GET /api/plan`, can trigger `POST /api/plan` to regenerate. Header is plain text (no card, no icon tile): "Weekly Plan" h2 (or "Your first week" + amber "DRAFT" badge — see **First-plan draft state** below) + "Week of …" subtitle with prev/next navigation arrows, with the "Generate Plan" button at right only when no plan exists yet (once a plan exists there's no header button — see **Quick-action steering** below). Shows: "Week in Review" card (latest auto-reflection summary + up to 3 lessons, only when < 10 days old), the quick-action steering row, "Priority This Week" card, day-by-day schedule (no difficulty chip), the "Start this plan" bar (draft only), and "Adjustments from last week" when present. (Next best action and strategy notes are returned by the API but not displayed.)

**First-plan draft state:** the mountain's very first weekly plan is created with `plan.status: "draft"` (stamped server-side in `POST /api/plan` when no prior plan exists for the mountain — every plan after that is active immediately). While draft: the h2 reads "Your first week" with a "DRAFT" badge, and a line under it reads "I made a starting plan based on your goal. Adjust anything before you begin." Day pills, the status-picker, and the Log this day/Finish today controls are all hidden — nothing is "tracked" yet. A green bar below the schedule ("Ready when you are — starting locks today in as day one." + **Start this plan →**) commits the draft: it clears `plan.status`, after which the view is indistinguishable from any other active week (pills, status picker, and check-in controls all appear).

**Quick-action steering:** a row of one-click chips sits above "Priority This Week" whenever the viewed week has at least one unfinished day (draft or active — this is not draft-only). **Make it lighter**, **Different approach**, and **Regenerate** each call `POST /api/plan/steer` directly with no input required; **Change my availability** opens a small inline popover (three presets — "Less time this week" / "About the same" / "More time this week" — plus a free-text field + Apply) before calling the same endpoint with `available_time`. The steer endpoint only rewrites days that aren't `finished` — logged history is never touched — and returns a merged plan the row applies immediately, with a one-step "⟲ Undo" toast (bottom-center, 8s) restoring the pre-action snapshot via `PATCH /api/plan`. A chip is disabled (spinner in place of its icon) while any steer call is in flight. "Discuss with AI" is demoted to a small text link at the right of this row ("Something more specific? Discuss with AI") rather than a primary button — it still opens `MiniGuideChat` in plan-talk mode.

**Per-task Edit / Replace / Skip:** on any task in a day that isn't `finished` (draft or active), hovering (or focusing, for keyboard users) the task card reveals three small icons top-right: **✎ Edit** turns the task into an inline text + duration field (Enter/Save commits, Escape/Cancel discards); **↻ Replace** expands a "Swap this for…" panel below the card with 2 AI-generated alternatives from `POST /api/plan/replace-task` (one more hands-on, one smaller/lower-effort) plus a "Something else" link that fetches one further alternative into the same panel — picking one applies it in place; **✕ Skip** removes the task immediately. Skip and Replace both trigger the same one-step undo toast as the quick-action chips; Edit does not (it's the user's own typed change, not an AI action to second-guess). Each unfinished day also gets a dashed "+ Add task" button at the bottom of its task list, which inserts a blank task already in edit mode. All of these autosave via the existing `PATCH /api/plan`.
...
**Changing the plan:** the primary path is now the quick-action row and per-task Edit/Replace/Skip described above — both apply instantly with no chat round-trip. "Discuss with AI" (the demoted text link) remains for changes that need explaining rather than reacting to: it opens `MiniGuideChat` in plan-talk mode with a summary of the current week; the guide asks what to change and regenerates the plan via its `propose_plan` action. The schedule on the page refreshes live (`refreshKey` prop bumped by the overview page) and a "✓ Your weekly plan is updated" note appears in the chat.
