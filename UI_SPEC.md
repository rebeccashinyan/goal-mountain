...
- Day status pill, centered under each day name: "✓ Day complete" (forest tint — when finished, or a rest/no-task day that has arrived/passed), "In progress" (amber — today, unfinished), "Upcoming" (muted — future days), "Not logged" (muted — past days with tasks never checked in). The old "✓ Day complete — nice climbing" footer is gone; today's finished card shows only a small centered "Reopen day" link.
