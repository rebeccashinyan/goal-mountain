...
**The viewed week is seeded once, never re-snapped.** `fetchPlan` sets `viewedWeekStart` only when it is still null (`w ?? mondayOf(new Date())`). It refetches whenever `refreshKey` changes — bumped by `onPlanUpdated` after a guide-driven plan change, and on any remount — and resetting the week there would silently move the user off the week they navigated to while the form and "Generate Plan" still pointed at it, so the next generate would create a plan for the *current* week instead of the one on screen.
...
- **Any day already in the past is muted**, even though the draft is untouched and unstarted — see "Before-you-started columns" below. A draft left sitting for days doesn't read as if its stale days were skippable-but-still-live tasks.
- A green bar below the schedule ("Nothing is tracked until you start — take as long as you need to adjust it.") holds two actions with deliberate hierarchy: **Start this week →** is the solid forest-green primary CTA, and **Change plan setup** sits beside it as a quiet ghost/text button (no fill, no border, lighter weight and 70% opacity) so it reads as the escape hatch rather than competing with the main path. On mobile they stack with the primary on top (`flex-col-reverse`). Nothing destructive lives in this bar — see **Discard draft** below.
- **Start this week →** commits the draft via `POST /api/plan/rebase`. If the draft has tasks on days that have already passed, that same call rescues them first — moving necessary ones to today or later, dropping optional ones — then activates, all in one click with no extra confirmation (see AGENTS.md → Planning Agent). Tracking begins at that moment and the view becomes a normal active week. When a rebase actually changed something, a toast appears exactly like any other AI-driven single action in this view: *"Plan adjusted to start today · 2 tasks moved · 1 optional task removed"* with **Undo**, dismissing after 8s. Starting exactly on time shows no toast at all — silent activation, same as before this behavior existed.
- **Change plan setup** reopens the same availability/constraints form used for a first plan, replacing the draft view, **pre-filled from `plan.setup`** (what the user actually told the Planning Agent for this week) so it's a revision of their answers rather than a retype. It offers **Generate new draft** (primary) and **Cancel — back to draft** (text). The existing draft is never deleted or modified on entering setup: generation INSERTs a new row and the newest-row-per-week rule swaps it in only once it succeeds, so a failed generation or a Cancel returns to the original draft with all Edit/Replace/Remove work intact. A failed generation shows "That didn't generate. Your current draft is untouched — try again, or go back to it."
- **Setup state is per-week, not per-component.** Availability and constraints genuinely differ week to week, so the form's open/error flags and both field values are held as a single object stamped with the `week_start` they were entered against; anything whose stamp doesn't match the viewed week isn't this week's state at all. Each week therefore falls back to its *own* `plan.setup` (or empty, when it has no plan yet), and navigating with the prev/next arrows never carries one week's typed answer onto another — nor leaves the "Cancel — back to draft" affordance showing on a week that has no draft. Returning to the week you were editing restores what you had typed, so an accidental arrow click costs nothing.
- **Discard draft** deliberately does *not* live in the footer — it's in the `···` overflow menu below "Regenerate the whole week", in summit red, subtitled "Deletes this week entirely. Nothing to come back to." It expands in place into a confirm ("Delete this draft and its edits? This can't be undone." → **Discard** / **Keep it**) and calls `DELETE /api/plan?plan_id=…`, returning the week to the no-plan state. It is a separate concern from Change plan setup: one replaces the draft, the other destroys it, so they must not sit at the same level in the same bar.
...
|------|-----|---------|-----------|
| 1 | One task is wrong | inline **Edit / Replace / Remove**, `+ Add task` | Edit/Remove/Add apply instantly with an undo toast; Replace previews first and keeps the task's duration, day, and priority (content-only change) |
...
- **↻ Replace** — a two-step AI-assisted flow in a "Replace with…" panel below the card, never a jump to Discuss with AI. **Replace changes a task's content, not its cost:** the original duration, day, priority, and the day's remaining capacity are preserved by default, so swapping a 20-min task's direction can't quietly turn it into a 2-hour one. Deliberately changing time belongs to **Edit** (this one task) or **Change my availability** (the week).
  ...
  2. **Preview.** Either path calls `mode: "generate"` with the chosen direction, which returns a task scoped to fit the original duration — concretely narrowed ("pick 3 listings, capture 6 quick notes each, ~6 min per listing"), not a maximal version at a longer estimate — shown in a bordered preview card headed "PREVIEW REPLACEMENT" with the **unchanged original duration** beneath it. Nothing has changed yet. **"Replace task"** (primary) commits it via `PATCH /api/plan`, scoped to only this one task; **"Back"** returns to the direction list (no re-fetch — the original 2-3 directions are kept) without discarding the panel.
  3. **Longer-version opt-in (only when the direction genuinely doesn't compress).** The `generate` response may include a `fullVersion`; most directions fit fine and omit it. When present, an amber sub-card sits inside the preview: *"A full version would take ~4 hours."* with two toggles — **Fit into 20 min** (pre-selected, dark) and **Use full 4 hours** (secondary, outlined) — and a live capacity line beneath, *"Using the full version makes Monday total 4h 45m."*, summed client-side from that day's other tasks. Toggling swaps which version the preview card shows; only the selected one is applied on confirm. This is the sole route by which Replace can lengthen a task, and even then **nothing else on the day is moved, shortened, or rescheduled to make room** — the capacity line states the consequence before committing and the user resolves it themselves if they want to.
  4. **Multi-task impact.** The `generate` call also flags any *other* task already in the plan that the new direction makes inconsistent (`affected`). If non-empty, confirming the replacement surfaces an amber banner above "Priority This Week": *"This choice also affects N later task(s). Update them too?"* with **Review changes** (calls the same `POST /api/plan/steer` the quick-action chips use, `action: "custom"`, scoped to only the flagged tasks — surfacing as the normal pending-revision card) and **Not now** (dismisses; those tasks are left untouched). The affected tasks are never changed by the replace itself.
...
- **Before-you-started columns:** the same visual treatment (dashed border, muted label) but a separate condition and separate cause — a day that *did* get scheduled tasks, but has already passed while the plan sat unstarted. While the plan is still a **draft**, this is checked live against today's date, so it updates the moment a day rolls past without ever touching the draft's own data — nothing is rebased until "Start this week" is actually pressed. Once **active**, the boundary is `plan.activated_from` instead (fixed at whatever today was when Start was pressed), so it stays muted permanently rather than reading as missed work — see `POST /api/plan/rebase` in AGENTS.md, which is what moves or drops the tasks that were sitting there. Label reads **"Before you started"** to distinguish the cause from "Before this plan"; if a day would qualify for both (rare — only possible in the always-past `plan_start_date` case), "Before this plan" takes precedence since it's the more specific reason.
