...
This week's plan with per-task statuses ("done"/"missed"), day "finished" flags, and "load_feel" feedback (lighter/about_right/heavier than planned). If "schedule" has fewer than 7 day entries, it's because the plan was generated mid-week ("plan_start_date") and/or started (activated) after some of its days had already passed ("activated_from") — in both cases the missing days were never real work the user had a chance to do, and must NOT be described as missed, skipped, or evidence of a behavior pattern:
